...
if(isset($_POST['repin'])) {
    ...
    if($result_csrf == false) {
        ...
        if($call->query("SELECT * FROM users WHERE phone = '$postPhone'")->num_rows == 0) {
            ...
            if($data_user['status'] == 'suspend') {
                ...
                $result_wa = $WATL->sendMessage($postPhone, "JAGA PIN KEAMANAN ANDA DAN JANGAN BERIKAN PIN KESIAPAPUN TERMASUK PIHAK {$_CONFIG['title']}\n\n_Personal Identification Number (PIN)_: *$PIN*\n\nMohon tidak memblokir nomor ini, hubungi customer service untuk bantuan.");
                $result_mail = mailer($_MAILER,[
                    'dest' => $data_user['email'],
                    'name' => $data_user['name'],
                    'subject' => 'PIN Baru '.$_CONFIG['title'].' Akun',
                    'message' => base64_encode(mailplate('repin',[
                        'name' => $data_user['name'],
                        'pin' => $PIN,
                    ])),
                    'is_template' => 'yes'
                ...
                if($result_mail == true || $result_wa == true) {
                $call->query("UPDATE users SET pin = '".bcrypt($PIN)."' WHERE phone = '$postPhone'");
                $_SESSION['result'] = ['type' => true,'message' => 'Kami telah mengirimkan PIN baru Ke email & whatsapp Anda.'];
                unset($_SESSION['pin']);
                redirect(0, base_url());
                } else {
                    $_SESSION['result'] = ['type' => false,'message' => 'Server is trouble, please contact admin.'];
                }
            }
        }
    }
} else if(isset($_POST['loginWhatsApp'])) {
    $postPhone = '0'.str_replace('-', '', filter($_POST['user']));
    ...
    if($result_csrf == false) {
        ...
        if($call->query("SELECT * FROM users WHERE phone = '$postPhone'")->num_rows == 0) {
            ...
            if($data_user['status'] == 'suspend') {
                ...
                $url = conf('WhatsApp',1);
                $Message = "JANGAN BERIKAN KODE INI KESIAPAPUN TERMASUK PIHAK {$_CONFIG['title']}\n\nMasuk ke akun dengan *Kode Verifikasi (OTP): $OTP*\n\nMohon tidak memblokir nomor ini, hubungi customer service untuk bantuan.";
                $data = [
                ...
                if($response['status'] == true) {
                    ...
                    $_SESSION['result'] = ['type' => true,'message' => 'Kode Verifikasi Telah Dikirim, Silahkan Cek WhatsApp Anda'];
                } else {
    ...
    $post_referral = !$post_referral ? 'MYH-SYSTEM' : filter($_POST['reff']);
    ...
    } else if(strtolower($post_referral) !== 'myh-system' && $call->query("SELECT * FROM users WHERE referral = '$post_referral'")->num_rows == false) {
        $_SESSION['result'] = ['type' => false,'message' => 'Kode Referral Tidak Valid.'];
    } else {
        $OTP = rand(111111,999999);
        $post_invited = (strtolower($post_referral) == 'myh-system') ? 'MYH-SYSTEM' : $call->query("SELECT * FROM users WHERE referral = '$post_referral'")->fetch_assoc()['username'];
        $post_invitid = (strtolower($post_referral) == 'myh-system') ? 'MYH-SYSTEM' : $call->query("SELECT * FROM users WHERE referral = '$post_referral'")->fetch_assoc()['name'];
        // $result_wa = $WATL->sendMessage($post_phone, "Hallo *$post_name*, \nJANGAN BERIKAN KODE INI KESIAPAPUN TERMASUK PIHAK {$_CONFIG['title']}\n\nAktivasi  akun dengan *Kode Verifikasi (OTP): $OTP*\n\nMohon tidak memblokir nomor ini, hubungi customer service untuk bantuan.");
        $result_mail = mailer($_MAILER, [
            'dest' => $post_email,
            'name' => $post_name,
            'subject' => 'OTP Untuk '.$_CONFIG['title'].' Akun',
    ...
    if($result_csrf == false) {
       ...
       $_SESSION['result'] = ['type' => false,'message' => 'Kode Otp Yang Anda Masukkan Salah, Harap Periksa Kembali.']; 
        ...
        $in = $call->query("INSERT INTO users VALUES ('','$card','$post_name','".$data['email']."','$post_phone','".$data['user']."','0','0','0','Basic','$kodeReff','$reff','active','default.png','No','$datetime', '".bcrypt($data['pin'],10)."')");
        if($in == true){
            ...
            $WATL->sendMessage($data['phone'],"Hallo ".$data['name']." \nSelamat datang di WhatsApp *{$_CONFIG['title']}* \nMelalui kanal ini, \nAnda akan menerima informasi berupa notifikasi terkait akun dan transaksi Anda di *{$_CONFIG['title']}* \n\n*MOHON JANGAN DI BLOKIR KONTAK INI*");
    ...
    if($result_csrf == false) {
        ...
        $result_wa = $WATL->sendMessage($data['phone'],"Hallo ".$data['name'].", \nJANGAN BERIKAN KODE INI KESIAPAPUN TERMASUK PIHAK {$_CONFIG['title']}\n\nMasuk ke akun dengan *Kode Verifikasi (OTP): ".$_SESSION['register']['otp']."\n\nMohon tidak memblokir nomor ini, hubungi customer service untuk bantuan.");
        $result_mail = mailer($_MAILER,[
            'dest' => $data['email'],
            'name' => $data['name'],
            'subject' => 'OTP Untuk '.$_CONFIG['title'].' Akun',
