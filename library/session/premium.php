...
if(isset($_SESSION['user'])) {
    ...
    if($call->query("SELECT * FROM users WHERE username = '$sess_username'")->num_rows == 0) {
        $_SESSION['result'] = ['type' => false,'message' => 'Fitur Tersebut Khusus Pengguna Neupay'];
        redirect(0,base_url('auth/logout'));
    } else if(!in_array($data_user['level'],['Admin','Premium','Basic'])) {
        $_SESSION['result'] = ['type' => false,'message' => 'Fitur Tersebut Khusus Level Premium'];
        redirect(0,base_url());
    }
}