...
class Flip
{
    private $base = 'https://bigflip.id/big_sandbox_api/v2';
    private $refId;
    private $user;
    private $key;
    ...
    public function __construct($punten) {
        ...
        $this->user = $punten['apikey'];
        $this->key = $punten['token'];
    }

    public function CheckBalance() {
        $data = $this->connect($this->base.'general/balance',['Content-Type: application/json'],json_encode([
            'cmd' => 'balance',
            'username' => $this->user,
            'sign' => md5($this->user.$this->key.'balance')
        ]))['data'];
