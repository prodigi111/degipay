...
if(isset($_POST['add'])) {
    ...
    } else if(!in_array($post_type,$allowed_type) || !$post_name) {
        $_SESSION['result'] = ['type' => false,'message' => 'There are still empty columns.'];
    } else if($call->query("SELECT name FROM category_donasi WHERE name = '$post_name')")->num_rows == true) {
        $_SESSION['result'] = ['type' => false,'message' => 'Category already exists.'];
    } else {
        $up = $call->query("INSERT INTO category_donasi VALUES ('','$post_name','$post_type')");
        if($up == TRUE) {
    ...
    if($result_csrf == false) {
        ...
        $_SESSION['result'] = ['type' => false,'message' => 'There are still empty columns.'];
    } else if($call->query("SELECT id FROM category_donasi WHERE id = '$post_token'")->num_rows == false) {
        $_SESSION['result'] = ['type' => false,'message' => 'Category not found.'];
    } else {
        $up = $call->query("UPDATE category_donasi SET name = '$post_name', type = '$post_type' WHERE id = '$post_token'");
        if($up == TRUE) {
    ...
    if($result_csrf == false) {
        ...
        $up = $call->query("DELETE FROM category_donasi WHERE id = '$post_token'");
        if($up == TRUE) {
                     ...
                     <div class="iq-card">
                        <div class="iq-card-header d-flex justify-content-between">
                           <div class="iq-header-title">
                              <h4 class="card-title" title="Click to Add" onclick="modal('New Data','<?= base_url('admin/donasi/add-category') ?>','','lg')"><i class="fas fa-plus-circle mr-2"></i> Category </h4>
                           </div>
                        </div>
...
if(isset($_GET['show'])) {
    ...
    $search = "SELECT * FROM category_donasi WHERE name LIKE '%$pt_search%' OR name LIKE '%$pt_search%' ORDER BY name ASC";
} else {
    $search = "SELECT * FROM category_donasi ORDER BY id ASC";
}
                                        ...
                                        <form method="POST" class="form-inline" role="form">
                                            ...
                                            <td>
                                                <select class="form-control" name="type">
                                                    <?php
                                                    $s = $call->query("SELECT * FROM category_donasi GROUP BY type ORDER BY type ASC");
