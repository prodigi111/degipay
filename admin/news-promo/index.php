    ...
    if($result_csrf == false) {
        ...
        if($ukuran < 2044070){      
            ...
            if(move_uploaded_file($_FILES['image']['tmp_name'], $_SERVER['DOCUMENT_ROOT'].'/library/assets/images/news/'.$rand.'_'.$filename)) {
                if($expired_at) {
                    $up = $call->query("INSERT INTO news_promo (title, description, banner, expired_at) VALUES ('$title', '$description', '$xx', '$expired_at')");
                } else {
                    $up = $call->query("INSERT INTO news_promo (title, description, banner) VALUES ('$title', '$description', '$xx')");
                }
                if($up == TRUE) {
        ...
        if($_FILES['image']['name'] != '') {
            if(!in_array($ext,$ekstensi)) {
                ...
                if($ukuran < 2044070){      
                    ...
                    if(move_uploaded_file($_FILES['image']['tmp_name'], $_SERVER['DOCUMENT_ROOT'].'/library/assets/images/news/'.$rand.'_'.$filename)) {
                        if($expired_at != 0) {
                            $up = $call->query("UPDATE news_promo SET title = '$title', description = '$description', expired_at = '$expired_at', banner = '$xx' WHERE id = '$id'");
                        } else {
                            $up = $call->query("UPDATE news_promo SET title = '$title', description = '$description', banner = '$xx', expired_at = NULL WHERE id = '$id'");
                        }
                        if($up == TRUE) {
                     ...
                     <div class="iq-card">
                        <div class="iq-card-header d-flex justify-content-between">
                           <div class="iq-header-title">
                              <h4 class="card-title" title="Click to Add" onclick="modal('Publish Event','<?= base_url('admin/news-promo/add') ?>','','lg')"><i class="fas fa-plus-circle mr-2"></i> News & Promo </h4>
                           </div>
                        </div>
...
?>
                                    <tr>
                                        <form method="POST" class="form-inline" role="form">
                                            ...
                                            <td align="center">
                                                <a href="javascript:;" onclick="modal('Edit Event','<?= base_url('admin/news-promo//edit?id='.$row_query['id']) ?>','','md')" class="btn btn-sm btn-warning text-white">
