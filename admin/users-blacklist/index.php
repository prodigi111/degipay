...
require _DIR_('admin/users-blacklist/action');
                     ...
                     <div class="iq-card">
                        <div class="iq-card-header d-flex justify-content-between">
                           <div class="iq-header-title">
                              <h4 class="card-title" title="Click to Add" onclick="modal('New Blacklist','<?= base_url('admin/users-blacklist/add') ?>','','lg')"><i class="fas fa-plus-circle mr-2"></i> User Blacklist </h4>
                           </div>
                        </div>
...
if(isset($_GET['show'])) {
    ...
    $search = "SELECT * FROM users_blacklist WHERE username LIKE '%$pt_search%' OR phone LIKE '%$pt_search%' ORDER BY id DESC";
} else {
    $search = "SELECT * FROM users_blacklist ORDER BY id DESC";
}
...
?>
                                    <tr>
                                        ...
                                        <td align="center">
                                            <a href="javascript:;" onclick="modal('Detail Blacklist','<?= base_url('admin/users-blacklist/detail?q='.base64_encode($row_query['username'])) ?>','','md')" class="btn btn-sm btn-primary">
                                                <i class="fas fa-list" title="Detail Blacklist"></i>
                                            </a>
                                            <a href="javascript:;" onclick="modal('Delete Blacklist','<?= base_url('admin/users-blacklist/delete?q='.base64_encode($row_query['username'])) ?>','','md')" class="btn btn-sm btn-warning text-white">
                                                <i class="fas fa-trash-alt" title="Delete Balcklist"></i>
                                            </a>
                                        </td>
                                    </tr>
<? } ?>
