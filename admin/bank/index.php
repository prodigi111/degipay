    ...
    if($result_csrf == false) {
        ...
        if($up == true) {
            if($post_status == 'success'){
                $WATL->sendMessage($rowUser['phone'], "Hallo, *{$rowUser['name']}*  TRANSFER SALDO BERHASIL\nID TRANSFER *#{$post_wid}* \nPesanan : *{$row['name']}* \nStatus : *".ucwords($post_status)."* \nCatatan: *{$post_note}* \n----------------------------------------------\n*Terima kasih sudah memilih {$_CONFIG['title']}*");
            }
...
if(isset($_GET['show'])) {
    ...
    $search = "SELECT * FROM trx WHERE trxtype IN ('bank') AND status LIKE '%$pt_search%' AND wid LIKE '%$pt_search2%' ORDER BY id DESC";
} else {
    $search = "SELECT * FROM trx WHERE trxtype IN ('bank') ORDER BY id DESC";
}
...
?>
                                        <tr>
                                            <form method="POST" class="form-inline" role="form">
                                                ...
                                                <td><span class="badge badge-primary"><?= $tab_icon.$row_query['wid'] ?></span></td>
                                                ...
                                                <td><span class="badge badge-success"><?= $row_query['tid'] ?></span></td>
                                                <td style="min-width:250px;">
                                                    <textarea class="form-control" name="note"><?= $row_query['note'] ?></textarea>
                                                </td>
                                                <?= $total = $row_query['price'] - $admin ?>
                                                <td><span class="badge badge-danger"><?= 'Rp '.currency($total) ?></span></td>
                                                <td>
