...
$search_cat = $call->query("SELECT * FROM category WHERE name = '$get_s' AND type IN ('voucher')");
if($search_cat->num_rows == 0) exit(redirect(0,base_url('order/voucher-domino')));
...
">
            <div class="wide-block-service">
                            ...
                            <div class="in">
                                <div class="form-group basic">
                                    <div class="input-wrapper">
                                        <label class="label">ID PELANGGAN/ID PENGGUNA</label>
                                        <input type="number" class="form-control" placeholder="Masukkan Nomor HP Terlebih Dahulu" name="data" id="data" required>
                                        <i class="clear-input">
                ...
                <ul class="nav nav-tabs style1 mt-1" role="tablist" id="tabpanel">
                    <li class="nav-item">
                        <a class="nav-link active" data-toggle="tab" href="#Server2" role="tab">
                            Higss Domino
                        </a>
                    </li>
                    <!--
                    <li class="nav-item">
                        <a class="nav-link" data-toggle="tab" href="#Server1" role="tab">
                            Server 2
                        </a>
                    </li>
                    -->
        ...
        <div class="tab-content">
            <div class="tab-pane fade" id="Server1" role="tabpanel">
                <div class="section rgs-list-layanan">
                    <?php
                    $search = $call->query("SELECT * FROM srv WHERE type IN ('voucher', 'voucher-data','tv') AND brand = '$operator' AND provider = 'DIGI' ORDER BY price ASC");
            ...
            <div class="tab-pane fade show active" id="Server2" role="tabpanel">
                <div class="section rgs-list-layanan">
                    <?php
                    $search = $call->query("SELECT * FROM srv WHERE type IN ('voucher', 'voucher-data','tv') AND brand = '$operator' AND provider = 'X' ORDER BY price ASC");
                    if($search->num_rows == FALSE) { ?>
                    <div class="alert alert-danger text-left fade show" role="alert">
                        Tidak Ada Layanan Yang Tersedia!
                    </div>
                    ...
                    <div class="row rgs-show">
                        ...
                        ?>
                        <div class="col-12 mb-2<?= $row['status'] == 'available' ? '' : ' rgs-layanan-gangguan' ?>">
                            <a href="javascript:;" onclick="ConfirmModal('<?= base_url('confirm-manual/'.$row['code'].'/') ?>')">
                                <div class="card">
                            <div class="card-body p-0" align="right">
        ...
        <div class="section rgs-list-layanan">
            <?php
            $search = $call->query("SELECT * FROM srv WHERE type IN ('voucher', 'voucher-data','tv') AND brand = '$operator' ORDER BY price ASC");
            if($search->num_rows == FALSE) { ?>
            <div class="alert alert-danger text-left fade show" role="alert">
                Tidak Ada Layanan Yang Tersedia!
            </div>
