...
$search_cat = $call->query("SELECT * FROM category WHERE name = '$get_s' AND type = 'voucher-tv'");
if($search_cat->num_rows == 0) exit(redirect(0,base_url('order/voucher-tv')));
...
">
            <div class="wide-block-service">
                <ul class="listview image-listview no-line no-space flush">
                    <li>
                        <div class="item">
                            <img src="<?= assets('images/voucher-tv/'.$code.'.png').'?'.time() ?>" class="image" height="50px" width="50px">
                            <div class="in">
                            <div class="in">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active" id="Server1" role="tabpanel">
                <div class="section rgs-list-layanan">
                    <?php
                    $search = $call->query("SELECT * FROM srv WHERE type = 'voucher-tv' AND brand = '$operator' AND provider = 'DIGI' ORDER BY price ASC");
            ...
            <div class="tab-pane fade" id="Server2" role="tabpanel">
                <div class="section rgs-list-layanan">
                    <?php
                    $search = $call->query("SELECT * FROM srv WHERE type = 'voucher-tv' AND brand = '$operator' AND provider = 'X' ORDER BY price ASC");
                    if($search->num_rows == FALSE) { ?>
                    <div class="alert alert-danger text-left fade show" role="alert">
                        Tidak Ada Layanan Yang Tersedia!
                    </div>
        ...
        <div class="section rgs-list-layanan">
            <?php
            $search = $call->query("SELECT * FROM srv WHERE type = 'voucher-tv' AND brand = '$operator' ORDER BY price ASC");
            if($search->num_rows == FALSE) { ?>
            <div class="alert alert-danger text-left fade show" role="alert">
                Tidak ada layanan yang tersedia!
            </div>
