...
$search_cat = $call->query("SELECT * FROM category WHERE name = '$get_s' AND type IN ('inject-voucher')");
if($search_cat->num_rows == 0) exit(redirect(0,base_url('order/inject-vouchers/')));
...
">
                            ...
                            <div class="in">
                                <div class="form-group basic">
                                    <div class="input-wrapper">
                                        <label class="label">Nomor Voucher</label>
                                        <input type="number" class="form-control" placeholder="Masukkan Nomor Voucher" name="data" id="data" required>
                                        <i class="clear-input">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active" id="Server1" role="tabpanel">
                <div class="section rgs-list-layanan">
                    <?php
                    $search = $call->query("SELECT * FROM srv WHERE type IN ('inject-voucher') AND brand = '$operator' AND provider = 'DIGI' ORDER BY price ASC");
            ...
            <div class="tab-pane fade" id="Server2" role="tabpanel">
                <div class="section rgs-list-layanan">
                    <?php
                    $search = $call->query("SELECT * FROM srv WHERE type IN ('inject-voucher') AND brand = '$operator' AND provider = 'X' ORDER BY price ASC");
                    if($search->num_rows == FALSE) { ?>
                    <div class="alert alert-danger text-left fade show" role="alert">
                        Tidak Ada Layanan Yang Tersedia!
                    </div>
        ...
        <div class="section rgs-list-layanan">
            <?php
            $search = $call->query("SELECT * FROM srv WHERE type IN ('inject-voucher') AND brand = '$operator' ORDER BY price ASC");
            if($search->num_rows == FALSE) { ?>
            <div class="alert alert-danger text-left fade show" role="alert">
                Tidak Ada Layanan Yang Tersedia!
            </div>
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="row">
                                    ...
                                    <div class="col-4 text-right">
                                        <h6 font="text-primary">Rp <?= currency(price($data_user['level'], $row['price'], $row['provider'])) ?></h6>
                                    </div>
                                </div>
                            </div>
                            <div class="card-footer">
