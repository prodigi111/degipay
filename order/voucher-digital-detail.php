...
$search_cat = $call->query("SELECT * FROM category WHERE name = '$get_s' AND type = 'voucher-digital'");
if($search_cat->num_rows == 0) exit(redirect(0,base_url('order/voucher-digital')));
...
">
            <div class="wide-block-service">
                <ul class="listview image-listview no-line no-space flush">
                    <li>
                        <div class="item">
                            <img src="<?= assets('images/produk-icon/voucher-digital/'.$code.'.png').'?'.time() ?>" class="image" height="50px" width="50px">
                            <div class="in">
                            <div class="in">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active" id="Server1" role="tabpanel">
                <div class="section rgs-list-layanan">
                    <?php
                    $search = $call->query("SELECT * FROM srv WHERE type = 'voucher-digital' AND brand = '$operator' AND provider = 'DIGI' ORDER BY price ASC");
                        ...
                        <div class="card">
                            ...
                            <div class="card-body" style="padding:5px 15px 15px">
                                ...
                                <div class="col-5 text-right">
                                    <h5 class="text text-gold mb-0"><b><?= currency(conf('member', 1)) ?> Koin</b></h5>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="tab-pane fade" id="Server2" role="tabpanel">
                <div class="section rgs-list-layanan">
                    <?php
                    $search = $call->query("SELECT * FROM srv WHERE type = 'voucher-digital' AND brand = '$operator' AND provider = 'X' ORDER BY price ASC");
                    if($search->num_rows == FALSE) { ?>
                    <div class="alert alert-danger text-left fade show" role="alert">
                        Tidak Ada Layanan Yang Tersedia!
                    </div>
        ...
        <div class="section rgs-list-layanan">
            <?php
            $search = $call->query("SELECT * FROM srv WHERE type = 'voucher-digital' AND brand = '$operator' ORDER BY price ASC");
            if($search->num_rows == FALSE) { ?>
            <div class="alert alert-danger text-left fade show" role="alert">
                Tidak ada layanan yang tersedia!
            </div>
