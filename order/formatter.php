...
class BeautyFormatter
{
    ...
    public function DigiflazzService($data) {
        for($i = 0; $i <= count($data)-1; $i++) {
            ...
            if(in_array($oType,['VOUCHER BLANK']) && in_array($data[$i]['brand'], ['TELKOMSEL', 'INDOSAT', 'AXIS', 'SMART', 'TRI', 'XL'])) $Type = 'voucher-data';
            ...
            if(in_array($oType, ['TV']) && in_array($data[$i]['brand'], ['DECODER GOL','ORANGE TV','NEX & GARUDA','K-VISION DAN GOL','TRANSVISION'])) $Type = 'voucher-tv';
            if(in_array($oType, ['AKTIVASI VOUCHER']) && in_array($data[$i]['brand'], ['INDOSAT','AXIS','TRI'])) $Type = 'inject-voucher';
            if(in_array($oType,['DATA'])) $Type = 'paket-data';
            if(in_array($oType,['PAKET SMS & TELPON'])) $Type = 'paket-telepon';
            if(in_array($oType,['PLN'])) $Type = 'token-pln';
            if(in_array($oType,['E-MONEY'])) $Type = 'dompet-digital';
            if(in_array($oType, ['E-MONEY']) && in_array($data[$i]['brand'], ['BRI BRIZZI', 'MANDIRI E-TOLL', 'TAPCASH BNI'])) $Type = 'uang-elektronik';
            if(in_array($oType,['VOUCHER'])) $Type = 'voucher-game';
            if(in_array($oType, ['VOUCHER']) && in_array($data[$i]['brand'], ['TELKOMSEL', 'INDOSAT', 'AXIS', 'SMART', 'TRI', 'XL'])) $Type = 'voucher-data';
            if(in_array($oType, ['VOUCHER','E-MONEY']) && in_array($data[$i]['brand'], ['TIX ID', 'ITUNES US REGION', 'WIFI ID', 'ALFAMART VOUCHER', 'INDOMARET', 'ANCOL', 'CARREFOUR / TRANSMART', 'JUNGLELAND', 'H&M', 'SPOTIFY', 'VIDIO'])) $Type = 'voucher-digital';
