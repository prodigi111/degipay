...
$_POSTBank = isset($_POST['bank']) ? filter($_POST['bank']) : '';
$_POSTRek = isset($_POST['rek']) ? filter ($_POST['rek']) : '';
$_POSTNama = isset($_POST['nama']) ? filter ($_POST['nama']) : '';
...
if ($data_user['level'] == 'Basic') {
    ...
    $price = $_POSTNominal + $admin;
$_POSTHarga =isset($_POST['total']) ? filter($_POST['total']) : '';
if(!$price || !$_POSTRek) {
    $_SESSION['result'] = ['type' => false, 'message' => 'Masukkan Nominal atau Nomor Rekening Terlebih Dahulu!.'];
    redirect(0, base_url('transfer'));
} else {
    ...
    if(isset($_POST['transfer'])) { 
        ...
        if($result_csrf == false) {
            ...
            $post_note_snd = "Transfer Ke ".$post_bank."";
            $post_note_rcv = "Transfer Dari {$data_user['name']}";
            $in = $call->query("INSERT INTO transfer VALUES ('', '$post_rid', '$sess_username', '$post_bank', '-', '$post_rek', '$post_amnt', '$post_amnt', 'unpaid', '$datetime', '$datetime')");
            ...
            if($in == true) {
                ...
                $call->query("INSERT INTO mutation VALUES ('','$sess_username','-','$post_amnt','Transfer | #$post_rid | $post_bank', '$date', '$time', 'Deposit')");
                $call->query("INSERT INTO trx VALUES ('','$post_rid','$post_nama','$sess_username','$post_bank','Transfer Bank','$post_rek','Transaksi Sedang Diproses','$post_amnt','0','processing','0','0','0','WEB,$post_ip','bank','$date $time','$date $time','X')");
                //$call->query("UPDATE users SET balance = balance+$post_amnt WHERE username = '$post_user'");
                $call->query("INSERT INTO logs VALUES ('','$post_user','transfer-bank','$post_ip','$datetime')");
             ...
             /*   
                ...
                $FCM->sendNotif($fcm_token, $notification, $data);
    */
                $_SESSION['success'] = ['type' => true,'message' => "Anda Berhasil Mengirim Saldo Rp ".currency($post_amnt)." ke ".$post_bank.""];
                unset($_SESSION['csrf']);
            } else {
    ...
    <div class="section s-invoice">
        ...
        <? require _DIR_('library/session/result-mobile') ?>
            ...
            <ul class="listview rgs-listview-custom image-listview">
                <li>
                    <div class="item">
                        <div class="in">
                            <div class="text-muted"> Bank Tujuan</div>
                            <div class="rgs-text-rincian col-8"><?= $_POSTBank ?></div>
                        </div>
                    </div>
                </li>
                <li>
                    <div class="item">
                        <div class="in">
                            <div class="text-muted">No Rek</div>
                            <div class="rgs-text-rincian col-8"><?= $_POSTRek ?></div>
                        </div>
                    </div>
                </li>
                <li>
                    <div class="item">
                        <div class="in">
                            ...
                            <div class="rgs-text-rincian col-8"><?= $_POSTNama ?></div>
                        </div>
                    </div>
                </li>
            </ul>
        ...
        <div class="listview-title mt-1">Detail Transaksi</div>
            ...
            <form method="POST">
                ...
                <input type="hidden" name="rek" value="<?= $_POSTRek ?>">
                <input type="hidden" name="bank" value="<?= $_POSTBank ?>">
                <input type="hidden" name="nama" value="<?= $_POSTNama ?>">
